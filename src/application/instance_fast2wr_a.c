    ...
    switch (inst->testAppState)
		{
			case TA_INIT :
			{
				...
				dwt_setrxtimeout(0);    //If set to 0 the timeout is disabled.
        ...
        case TA_TXRANGINGINIT_WAIT_SEND :
                ...
                //ychange
                	...
                	memset(dataseq, ' ', 40);
                	memcpy(dataseq, (const uint8 *) "      rngint   ", 15);
                	writetoLCD( 15, 1, dataseq); //send some data
*/
                	//uint8 steps = rndCount % 10;
                ...
                //RANGE INIT uses delayed send...so set the appropriate delay time in register
                ...
                inst->msg_f.messageData[PCODE] = RESP_PASS_CODE; // response password
                    ...
                    else
                    {
                        ...
                        switch(fcode)
                        {
                            ...
                            case RTLS_DEMO_MSG_TAG_POLLF:
                            {
								...
								memcpy(&(inst->encryption_data.key), &messageData[PCODE], POLL_KEY_LEN);
                            ...
                            case RTLS_DEMO_MSG_TAG_FINALF:
                            {
                                ...
                                uint32 diffRmP_encr = encrypt_rc4( &(inst->encryption_data.key), (uint32)rtd.diffRmP);
                                ...
                                pollRespRTD = (diffRmP_encr - (uint32)aTxT);
                            	...
                            	writetoLCD( 1, 0,  &command);
                            	memset(dataseq, ' ', 40);
                            	memcpy(dataseq, (const uint8 *) "      finalf   ", 15);
                            	writetoLCD( 15, 1, dataseq); //send some data
									*/
                            	//---
