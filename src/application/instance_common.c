...
void instance_config()
{
	...
	//Configuration for DecaRanging Modes (only mode 010 of EVB1000 being used)
	chConfig_t config =
	{
		2,              // channel
		DWT_PRF_16M,    // prf
    ...
    instance_data[instance].configTX.PGdly = txSpectrumConfig[config.channel].PGdelay ;

    //firstly check if there are calibrated TX power value in the DW1000 OTP
    power = dwt_getotptxpower(config.prf, instance_data[instance].configData.chan);

    if((power == 0x0) || (power == 0xFFFFFFFF)) //if there are no calibrated values... need to use defaults
    {
        power = txSpectrumConfig[config.channel].txPwr[config.prf- DWT_PRF_16M];
    }
    ...
    {
        instance_data[instance].txantennaDelay = rfDelays[config.prf - DWT_PRF_16M];
        // -------------------------------------------------------------------------------------------------------------------
    ...
    else
    {
        ...
        instance_data[instance].txantennaDelay = dwt_readantennadelay(config.prf) >> 1;
    ...
    if(config.preambleLength == DWT_PLEN_64) //if preamble length is 64
	{
